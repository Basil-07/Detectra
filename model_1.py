...
def prepare_training_data(drug_files, non_drug_files):
    ...
    for i, path in enumerate(non_drug_files):
        try:
            ...
            analyze_spectrum(df, f"non_drug_{i+1}")
...
if __name__ == "__main__":
    ...
    # Corrected non_drug_files to reflect uploaded files (glucose.csv instead of sucrose.csv)
    non_drug_files = [
        "lactic.csv", 
        "citric.csv", 
        "ethanol.csv", 
    ...
    print("🚀 Starting Drug Detection System")
    ...
    try:
        ...
        # Set the test file to cocaine.csv
        file = "cocaine.csv" 
        result = predict_sample(file, binary_model, multiclass_model, le)
